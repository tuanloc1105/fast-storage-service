...
func KeycloakLogin(ctx context.Context, username string, password string) (payload.ProtocolOpenidConnectToken, error) {
	...
	loginCurlCommand := fmt.Sprintf(
		"curl -k --location '%s/realms/master/protocol/openid-connect/token' --header 'Content-Type: application/x-www-form-urlencoded' --data-urlencode 'client_id=%s' --data-urlencode 'client_secret=%s' --data-urlencode 'username=%s' --data-urlencode 'password=%s' --data-urlencode 'grant_type=password' --data-urlencode 'scope=openid'",
